...
import { Pedido, ItemPedido, StatusPedido, STATUS_PEDIDO_CONFIG, PEDIDO_STATUS_FLOW } from "@/types/lanchonete";
import { pedidos as pedidosIniciais, produtosLanchonete } from "@/data/lanchoneteData";
...
export default function LanchonetePedidos() {
  const [orders, setOrders] = useState<Pedido[]>(pedidosIniciais);
  const [isNewOrderOpen, setIsNewOrderOpen] = useState(false);
  const [clientName, setClientName] = useState("");
  const [selectedItems, setSelectedItems] = useState<ItemPedido[]>([]);

  const updateOrderStatus = (orderId: string, newStatus: StatusPedido) => {
    setOrders((prev) =>
      prev.map((order) =>
  ...
  const handleCreateOrder = () => {
    ...
    toast({
      title: "Pedido criado",
      description: `Pedido #${newOrder.id.replace("ped-", "")} para ${newOrder.cliente_nome} foi criado.`,
    });
  };
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
              {produtosLanchonete.map((product) => (
                <Button
                  key={product.id}
                  variant="outline"
                  size="sm"
                  className="justify-start text-left h-auto py-2 px-3"
                  onClick={() => addItemToOrder({ id: product.id, nome: product.nome, preco: product.preco })}
                >
